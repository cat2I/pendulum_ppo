...
class CartPoleSwingUpEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # HÀM CHẤM ĐIỂM (ĐÃ FIX LỖI HỆ QUY CHIẾU)
        # Vì rủ xuống là 0 độ (cos=1), hướng lên là Pi (cos=-1)
        # Ta lấy -cos_th để: Hướng lên được +1 điểm, rủ xuống bị -1 điểm
        reward_theta = -cos_th 
        
        # Phạt nếu chạy xa trung tâm (để không đâm vào vách)
        penalty_x = 0.1 * (cart_x**2)
        
        # Phạt nhẹ nếu dùng lực quá lố (ép AI phải múa mượt mà)
        penalty_action = 0.001 * (action[0]**2)
        
        reward = float(reward_theta - penalty_x - penalty_action)
        # ==========================================
        
        # KẾT THÚC (TERMINATED): Nếu xe chạy quá 1 mét về hai phía thì vỡ ray
...
if __name__ == "__main__":
    ...
    # Đặt tên thư mục TensorBoard để dễ theo dõi và ép chạy bằng CPU cho nhanh
    ...
    model.learn(total_timesteps=500000, tb_log_name="Fix_Reward_20N")
