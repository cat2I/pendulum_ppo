...
if __name__ == "__main__":
    ...
    for current_seed in seeds:
        ...
        run = wandb.init(
            ...
            group="SB3_Stack8",       
            name=f"sb3_stack8_seed_{current_seed}",
            config={
                "total_timesteps": TOTAL_TIMESTEPS,
                "learning_rate": 3e-4,
                "architecture": "128x128",
                "seed": current_seed,
                "n_stack": 8  # log stack size for future reference
            },
        ...
        model.learn(total_timesteps=TOTAL_TIMESTEPS, 
                    tb_log_name=f"ppo_sb3_stack8_seed_{current_seed}", 
                    callback=WandbCallback())
        
        # force final state checkpointing
        os.makedirs("models/stack 8", exist_ok=True)
        os.makedirs("vec/stack 8", exist_ok=True)
        model.save(f"models/stack 8/ppo_force_real_stack8_seed_{current_seed}")
        vec_norm.save(f"vec/stack 8/vec_normalize_stack8_seed_{current_seed}.pkl")
